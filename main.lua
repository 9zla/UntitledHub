...
-- Removed the old RollingType/InstanceName check here to avoid alternating bug
-- (So no print on top, moved into queued teleport code instead)
...
-- UPDATED RejoinAndRollback with inline print after teleport
local function RejoinAndRollback(rollingType, instanceName)
    ...
    local teleport_code = string.format([[
        ...
        if getgenv().rollingType and getgenv().instanceName then
            if getgenv().rollingType == "Weapon" then
                warn('Was Rolling For: ' .. getgenv().instanceName)
            elseif getgenv().rollingType == "Trait" then
                warn('Was Rolling For: ' .. getgenv().instanceName)
            end
        end
...
------------------------------ // Auto-Resume Rolling After Rejoin (NEW) \\ ------------------------------
